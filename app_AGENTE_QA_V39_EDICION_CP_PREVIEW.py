...
# ============================================================
# EDITOR DEL CP NUEVO — REVISIÓN FUNCIONAL PREVIA A AZURE
# ============================================================
def render_new_cp_preview_editor(previews):
    """Editor del CP generado desde la HU. Solo modifica el PREVIEW."""
    preview_list = previews if isinstance(previews, list) else [previews]
    if not preview_list:
        return

    st.divider()
    st.markdown("## ✏️ CP nuevo — Revisión funcional")
    st.caption(
        "Lee y ajusta aquí el CP generado desde la HU antes de enviarlo a Azure. "
        "El Test Case real de Azure se usa únicamente como referencia estructural. "
        "Los cambios quedan en PREVIEW y no modifican Azure."
    )

    labels = []
    for idx, case in enumerate(preview_list):
        cid = safe_text(case.get("ID"), f"CP-PREVIEW-{idx + 1:05d}")
        title = safe_text(case.get("Title"), "Sin título")
        labels.append(f"{cid} — {title[:100]}")

    selected_label = st.selectbox(
        "Selecciona el CP para revisar y ajustar",
        labels,
        key="qa_preview_editor_selected_case",
    )
    selected_index = labels.index(selected_label)
    case = preview_list[selected_index]

    sections = dict(_reference_description_sections(case.get("Description", "")))
    product = safe_text(case.get("Product"), sections.get("Producto:"), "Pendiente")
    module = safe_text(case.get("Module"), sections.get("Módulo:"), "Pendiente")
    description = safe_text(
        sections.get("Descripción:"),
        case.get("Scenario"),
        "Pendiente",
    )
    expected = safe_text(
        sections.get("Resultado esperado de la prueba:"),
        case.get("Expected Result"),
        "Pendiente",
    )
    preconditions = safe_text(
        sections.get("Precondiciones:"),
        case.get("Preconditions"),
        "Pendiente",
    )
    related = safe_text(
        sections.get("Caso de uso relacionado:"),
        sections.get("Casos de Uso Relacionados:"),
        case.get("Related Use Case"),
        "Pendiente",
    )

    st.markdown(
        f"### {safe_text(case.get('ID'), f'CP-PREVIEW-{selected_index + 1:05d}')}"
    )
    st.text_input(
        "ID",
        value=safe_text(case.get("ID"), f"CP-PREVIEW-{selected_index + 1:05d}"),
        disabled=True,
        key=f"preview_id_{selected_index}",
    )

    new_title = st.text_input(
        "Título",
        value=safe_text(case.get("Title"), "Sin título"),
        key=f"preview_title_{selected_index}",
    )
    ...
    with c1:
        new_product = st.text_input(
            "Producto",
            value=product,
            key=f"preview_product_{selected_index}",
        )
    with c2:
        new_module = st.text_input(
            "Módulo",
            value=module,
            key=f"preview_module_{selected_index}",
        )
    ...
    new_description = st.text_area(
        ...
        value=description,
        height=150,
        key=f"preview_description_{selected_index}",
    )
    new_expected = st.text_area(
        "Resultado esperado de la prueba",
        value=expected,
        height=130,
        key=f"preview_expected_{selected_index}",
    )
    new_preconditions = st.text_area(
        "Precondiciones",
        value=preconditions,
        height=130,
        key=f"preview_preconditions_{selected_index}",
    )
    new_related = st.text_area(
        "Caso de uso relacionado",
        value=related,
        height=100,
        key=f"preview_related_{selected_index}",
    )

    st.markdown("### 🧪 Steps")
    st.caption(
        "Mantén la estructura aprobada de Azure: cada Step debe tener Action y Expected value."
    )

    steps = safe_steps(case)
    edited_steps = []
    remove_index = None

    for step_idx, step in enumerate(steps):
        step_number = safe_text(step.get("Step #"), str(step_idx + 1))
        with st.container(border=True):
            st.markdown(f"**Step {step_number}**")
            action = st.text_area(
                "Action",
                value=safe_text(step.get("Action")),
                height=80,
                key=f"preview_action_{selected_index}_{step_idx}",
            )
            expected_step = st.text_area(
                "Expected value",
                value=safe_text(step.get("Expected value")),
                height=80,
                key=f"preview_step_expected_{selected_index}_{step_idx}",
            )
            edited_steps.append({
                "Step #": step_number,
                "Action": action,
            ...
            if st.button(
                f"🗑️ Eliminar Step {step_number}",
                key=f"preview_remove_step_{selected_index}_{step_idx}",
            ):
                remove_index = step_idx

    if remove_index is not None:
        edited_steps.pop(remove_index)
        for idx, step in enumerate(edited_steps, start=1):
            step["Step #"] = str(idx)
        case["Steps"] = edited_steps
        preview_list[selected_index] = case
        st.session_state.azure_reference_preview = preview_list
        st.rerun()

    if st.button("➕ Agregar Step", key=f"preview_add_step_{selected_index}"):
        edited_steps.append({
            "Step #": str(len(edited_steps) + 1),
            "Action": "",
            "Expected value": "",
        })
        case["Steps"] = edited_steps
    ...
    with b1:
        if st.button(
            "💾 Guardar ajustes del CP",
            key=f"preview_save_{selected_index}",
            type="primary",
        ):
            ...
            updated_case.update({
                ...
                "Description": build_azure_description(
                    ...
                    related_use_case=new_related,
                ),
                "Expected Result": new_expected.strip(),
                "Preconditions": new_preconditions.strip(),
                "Related Use Case": new_related.strip(),
                "Steps": edited_steps,
            })
            preview_list[selected_index] = updated_case
            st.session_state.azure_reference_preview = preview_list
            st.success("✅ Ajustes guardados en el PREVIEW. No se modificó Azure.")
            st.rerun()

    with b2:
        if st.button(
            "🧹 Limpiar PREVIEW y volver a edición original",
            key=f"preview_clear_{selected_index}",
        ):
            st.session_state.azure_reference_preview = None
            st.session_state.azure_preview_edit_mode = False
            st.rerun()

    current_case = preview_list[selected_index]
    desc_text = safe_text(current_case.get("Description"))
    current_steps = safe_steps(current_case)
    checks = {
        "Producto dentro de Description": "producto:" in desc_text.lower(),
        "Módulo dentro de Description": (
            "módulo:" in desc_text.lower() or "modulo:" in desc_text.lower()
        ),
        "Descripción dentro de Description": (
            "descripción:" in desc_text.lower() or "descripcion:" in desc_text.lower()
        ),
        "Resultado esperado dentro de Description": (
            "resultado esperado de la prueba:" in desc_text.lower()
        ),
        "Precondiciones dentro de Description": "precondiciones:" in desc_text.lower(),
        "Caso de uso relacionado dentro de Description": (
            "caso de uso relacionado:" in desc_text.lower()
        ),
        "Steps con Action + Expected": bool(current_steps) and all(
            safe_text(s.get("Action")) and safe_text(s.get("Expected value"))
    ...
    if all(checks.values()):
        st.success("✅ El CP cumple la estructura aprobada y está listo para revisión funcional.")
    else:
        st.warning("⚠️ El CP todavía tiene elementos pendientes. Corrígelos antes de avanzar.")
...
if st.button(
    ...
    try:
        ...
        st.success(
            ...
            f"{len(result['TEST_CASES'])} casos desde la HU. "
            "Ahora vuelve a la sección 'Test Case de referencia' y pulsa "
            "'Generar CP nuevo para revisión funcional'."
        )
...
if result:
    ...
    # ========================================================
    # NUEVO EDITOR — CP GENERADO PARA REVISIÓN FUNCIONAL
    # ========================================================
    if st.session_state.get("azure_reference_preview"):
        render_new_cp_preview_editor(
            st.session_state.azure_reference_preview
        )

    if not st.session_state.get("azure_reference_preview"):
        # ========================================================

...
def _prepare_case(test_case):
    ...
    related = _text(draft.get("Related Use Case"))
    # Regla: un solo CU por CP.
    draft["_related_use_case"] = related
    ...
    draft["_steps_df"] = pd.DataFrame(
        rows or [{"Steps": 1, "Action": "", "Expected": ""}]
    )
    return draft
...
def _commit_case(draft):
    draft["Preconditions"] = "\n".join(
        x.strip() for x in draft.get("_preconditions", []) if x.strip()
    )

    # Un solo CU por Test Case.
...
def render_azure_style_editor(selected_case, selected_index):
    """Editor de Description y Steps con experiencia tipo Azure."""
    ...
    draft["Expected Result"] = st.text_area(
        ...
        value=_text(draft.get("Expected Result")),
        height=130,
        key=f"editor_expected_{selected_index}",
    )
    ...
    for i, value in enumerate(preconditions):
        preconditions[i] = st.text_input(
            f"Precondición {i + 1}",
            value=value,
            key=f"editor_pre_{selected_index}_{i}",
            label_visibility="collapsed",
        )

    if st.button(
        "＋ Agregar precondición",
        key=f"editor_add_pre_{selected_index}",
    ):
    ...
    draft["_related_use_case"] = st.text_input(
        "Caso de uso",
        value=draft["_related_use_case"],
        key=f"editor_related_{selected_index}",
    )
    ...
    with col_save:
        save = st.button(
            ...
            type="primary",
            use_container_width=True,
            key=f"editor_save_{selected_index}",
        )
    ...
    with col_cancel:
        cancel = st.button(
            "Cancelar",
            use_container_width=True,
            key=f"editor_cancel_{selected_index}",
        )
...
def delete_test_case(result, selected_index):
    """Elimina el CP seleccionado y su cobertura asociada."""
    ...
    if isinstance(coverage, list) and case_id:
        result["COVERAGE"] = [
            ...
            if _text(row.get("Test Case")) != case_id
        ]

    # Limpieza del estado del editor.
